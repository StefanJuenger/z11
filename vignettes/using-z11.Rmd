...
## Storing of the data
Admittedly, the package is massive, which is unfortunate, especially when updating it to a newer version in the future. I am still thinking about a smart option before installing whether users want to use something like an online or offline version of the package. In my opinion, an offline version is mandatory as some users maybe want to use the package in an offline environment. For example, in social science survey research, we often work in a secure environment to access sensitive data, comprising small-scale geographic information. These specialized workplaces do not have internet access, so it is beneficial to wrangle with the census data offline.
